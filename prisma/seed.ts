...
async function main() {
  ...
  for (let i = 0; i < cattleData.length; i++) {
    ...
    for (let w = 0; w < 5; w++) {
      const weightRecord = await prisma.cattleWeight.create({
        data: {
          cattleId: cattle.id,
      ...
      // Add 1 weight media per weight record
      await prisma.cattleWeightMedia.create({
        data: {
          weightId: weightRecord.id,
          fileUrl: `https://images.unsplash.com/photo-1547887537-6158d64c35b3?w=400&weight=${weightRecord.id}`,
          fileType: 'image/jpeg',
        },
      })
    }
    ...
    for (let h = 0; h < 2; h++) {
      const healthRecord = await prisma.cattleHealthRecord.create({
        data: {
          cattleId: cattle.id,
      ...
      // Add 1 health media per health record
      await prisma.cattleHealthMedia.create({
        data: {
          healthRecordId: healthRecord.id,
          fileUrl: `https://images.unsplash.com/photo-1516467508483-a7212febe31a?w=400&health=${healthRecord.id}`,
          fileType: 'image/jpeg',
        },
      })
    }
